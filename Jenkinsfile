pipeline {
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/miguelBarros1983/XML.git'
            }
        }
        ...
        stage('Restore') {
            steps {
                sh 'dotnet restore PrsingXMLFile/PrsingXMLFile.csproj'
            }
        }

        stage('Build') {
            steps {
                sh 'dotnet build PrsingXMLFile/PrsingXMLFile.csproj --no-restore'
            }
        }
